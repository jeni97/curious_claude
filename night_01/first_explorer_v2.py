#!/usr/bin/env python3
"""
NIGHT 2: Explorer with Diversity Incentive
Testing unique question generation with epistemic spiral
"""
...
"""
print("\n" + "="*70)
print("LOADED PROMPT (first 300 chars):")
print("="*70)
print(EXPLORER_PROMPT_UNIFIED[:300])
print("="*70 + "\n")


def run_explorer(explorer_id=1, temperature=None):
    """Run single Explorer with diversity incentive prompt"""
    ...
    print("   Model: " + MODEL)
    print("   Prompt: Diversity incentive + epistemic spiral")
    print(f"   Temperature: {temp}")
    print("   Expected: Unique question investigation")
    print("\n" + "-"*70)
    ...
    try:
        ...
        response = client.chat.completions.create(
            model=MODEL,
            messages=[
                {"role": "user", "content": EXPLORER_PROMPT_V2}
            ],
        ...
        result = {
            ...
            'reasoning': reasoning,
            'output': output,
            'prompt': EXPLORER_PROMPT_V2
        }
        
        # Save to JSON
        json_path = os.path.join(NIGHT_DIR, f"explorer_{explorer_id}_output_{timestamp}.json")
        with open(json_path, 'w') as f:
            json.dump(result, f, indent=2)
        
        # Save to readable text
        txt_path = os.path.join(NIGHT_DIR, f"explorer_{explorer_id}_output_{timestamp}.txt")
        with open(txt_path, 'w') as f:
            f.write("="*70 + "\n")
            f.write(f"CURIOUS CLAUDE - NIGHT 2 EXPLORER #{explorer_id} OUTPUT\n")
            f.write("="*70 + "\n\n")
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print("="*70)
        print("\nNext steps:")
        print("1. Review the unique question identified")
        print("2. Check if it differs from Night 1 question")
        print("3. Evaluate if epistemic spiral still occurred")
        print("4. Note if diversity incentive worked")
        print("5. Run 4 more Explorers with different temperatures")
        print("6. Compare questions for diversity")
    else:
        print("\n" + "="*70)
